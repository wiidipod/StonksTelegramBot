...
if __name__ == '__main__':
    ...
    for i, ticker_chunk in enumerate(ticker_chunks):
        ...
        highs, lows, closes, opens = yfinance_service.get_prices(ticker_chunk, period=period, interval='1d')

        for ticker in tqdm(ticker_chunk):
            high = highs[ticker][start_index:end_index]
            low = lows[ticker][start_index:end_index]
            ...
            if lower_growth_low[-future] < high[-1]:
                close_too_high += 1
                continue

            fair_value = yfinance_service.get_fair_value(ticker, growth_high[:-future], backtest=backtest)
            if fair_value is None or fair_value <= 0.0:
                no_fair_value += 1
                continue

            one_year_estimate = min(growth_low[-1], fair_value)
            if one_year_estimate <= high[-1]:
